...
function ListItem({todo, id, checkComplete}) {
  ...
  return (
    ...
    <div className="List_item"  htmlFor={id}  >
      ...
      <div>
        <button className="button_detail" disabled={todo.complete} onClick={() => handleOnEdit()}>Detail</button>
        <button className="button_delete"  onClick={() => handleDelete(todo.id)}>Remove</button>
      </div>
    </div>
    {
      onEdit && (
        <div autoComplete="off" className="form_update"  >
        <input type="text" name="todos" id="todos" className="text_input"
        ...
        <button className="button"  onClick={() => handleOnClick()}>Update</button>
      </div>
      )
    }
    </>
